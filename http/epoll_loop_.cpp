...
void do_write_(int cfd_,int epfd,int event,void *arg)
{
    ...
    if(strcmp(buf_p,"GET")==0)
    {
        ...
        decode_str((char*)path_,(char*)path_);
        char* pathv=(char*)path_;
        if(pathv[0]=='/')
        {
            pathv++;
        }
        sprintf(buf_path,"%s/%s",getcwd(buf_temp,SIZE),pathv);
        if(buf_path[strlen(buf_path)-1]=='/')
        {
        ...
        {
           char *p="是普通文件";
           send_respond_head(cfd, 200, "OK", get_file_type(buf_path), -1);
