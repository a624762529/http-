...
void getDiffrentDir(char *dir1,char *dir2,char *&dir3)
{
...
void send_dir(int cfd, const char* dirname)
{
    ...
    char *dir_=nullptr;
    //获取当前目录和工作目录的区分
    getDiffrentDir((char*)working_path,(char*)dirname,dir_);
    if((dir_!=NULL)&&dir_[0]=='/')
    {
        dir_++;
    }
    ...
    while( (ptr = readdir(dir)) != NULL )
    {
        ...
        if(dir_!=NULL)
        {
            sprintf(show_name,"%s/%s",dir_,name);;
        }
        else
        {
            sprintf(show_name,"%s",name);
        }
        if(strcmp(name,".")==0||strcmp(name,"..")==0)
            continue;
        sprintf(path_,"%s/%s",working_path,show_name);;
        //cout<<name<<endl;
        struct stat st;
        stat(path_,&st);
